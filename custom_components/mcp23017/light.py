"""Platform for mcp23017-based light."""
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    """Set up the MCP23017 for light entities."""
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    light_entity = MCP23017Light(hass, config_entry)
    light_entity.device = await async_get_or_create(
        hass, config_entry, light_entity
    )

    if await hass.async_add_executor_job(light_entity.configure_device):
        async_add_entities([light_entity])


async def async_unload_entry(hass, config_entry):
    """Unload MCP23017 light entry corresponding to config_entry."""
    _LOGGER.warning("[FIXME] async_unload_entry not implemented")


class MCP23017Light(LightEntity):
    ...
    def __init__(self, hass, config_entry):
        ...
        # Create or update option values for light platform
    ...
    @property
    def is_on(self):
        """Return true if the light is on, based on the binary sensor state."""
    ...
    def configure_device(self):
        ...
        if self.device:
            ...
            # Configure entity as output for a light
